...
{
	localized_tag_name: '可交易',
	internal_name: "tradable",
	category: "misc",
	localized_category_name: '杂项'
};

var kStandardTag_Untradable =
{
	localized_tag_name: '不可交易',
	...
	LoadContexts: function( rgAppContextData )
	{
		for ( var appid in rgAppContextData )
		{
			...
			if ( rgContextIds.length > 1 )
			{
				...
				var templAllContextName = new Template( '全部 #{appname} 物品');
...
var g_rgEconomyDisplay = {"440":{"howtoget":"\u60a8\u53ef\u4ee5\u901a\u8fc7\u514d\u8d39\u7684\u7269\u54c1\u6389\u843d\u7cfb\u7edf\u3001\u5728\u66fc\u6069\u516c\u53f8\u5546\u5e97\u8d2d\u4e70\u6216\u4e0e\u597d\u53cb\u4ea4\u6613\u6765\u83b7\u5f97\u5b83\u4eec\u3002"},"620":{"howtoget":"\u60a8\u53ef\u4ee5\u901a\u8fc7 Portal 2 \u6e38\u620f\u5185\u7f6e\u5546\u5e97\u6216\u4e0e\u597d\u53cb\u4ea4\u6613\u6765\u83b7\u5f97\u5b83\u4eec\u3002"},"753":{"1":{"howtoget":"\u60a8\u53ef\u4ee5\u5728\u7279\u4ef7\u4fc3\u9500\u4e2d\u5f97\u5230\u989d\u5916\u7684\u6e38\u620f\u526f\u672c\uff0c\u6216\u8005\u901a\u8fc7 Steam \u5546\u5e97\u4e70\u6e38\u620f\u5728\u4ed8\u6b3e\u65f6\u9009\u62e9\u201c\u4f5c\u4e3a\u793c\u7269\u8d2d\u4e70\u201d\u3002"}},"99900":{"logo":"https:\/\/community.fastly.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"},"99920":{"logo":"https:\/\/community.fastly.steamstatic.com\/public\/images\/economy\/applogos\/99900.png"}};
...
function ImageURL( imageName, x, y, bEnableHighDPI )
{
	if ( imageName )
	{
		...
		return 'https://static.wikia.nocookie.net/crossfire/images/c/c3/M82A1-BornBeast_Bagicon.png/revision/' + v_trim(imageName) + strSize + '?allow_animated=1';
	}
	else
...
var HoverCurrencyFromTemplate = new Template( '<span style="#{currencystyle}">#{amount}</span> 自 #{contextname}');
	...
	for ( var i = 0; i < rgDescriptions.length; i++ )
	{
		...
		else
		{
			...
			if ( description.type == 'usertext' )
			{
				...
				{
					strEscapedDescription = '<strong>自定义描述：</strong>' + strEscapedDescription;
				}
			}
...
function PopulateScrapAction( elActions, elScrapAmount, elScrapLink, item, appid, item_type, border_color )
{
	...
	$J.ajax({
		...
		success: function( response )
		{
			...
			{
				elScrapAmount.update( '%s 个宝石'.replace( /%s/, response.goo_value  ) );
...
function PopulateMarketActions( elActions, item )
{
	...
	if ( ( typeof(g_bViewingOwnProfile) != 'undefined' && g_bViewingOwnProfile ) || bIsTrading )
	{
		...
		elPriceInfoContent.update( '<img src="https://community.fastly.steamstatic.com/public/images/login/throbber.gif" alt="处理中...">' );
		...
		new Ajax.Request( 'https://steamcommunity.com/market/priceoverview/', {
			...
			onSuccess: function( transport ) {
				if ( transport.responseJSON && transport.responseJSON.success )
				{
					...
					{
						strInfo += '起价： ' + transport.responseJSON.lowest_price + '<br>'
					}
					else
					{
						strInfo += '此物品目前架上缺货。' + '<br>';
					}

					if ( transport.responseJSON.volume )
					{
						var strVolume = '在 24 小时内卖出了 %1$s 个';
						strVolume = strVolume.replace( '%1$s', transport.responseJSON.volume );
						//strInfo += '价格中位数： ' + transport.responseJSON.median_price + '<br>';
						strInfo += '个数： ' + strVolume + '<br>';
					}
			...
			new Ajax.Request('https://steamcommunity.com/market/canbuyback/', {
				...
				onSuccess: function (transport) {
					if ( transport.responseJSON && transport.responseJSON.canbuyback && transport.responseJSON.price ) {
						...
						$(buybackid).update('您可以在 2019 年 1 月 4 日前将此卡牌以 %price% 出售给 Artifact。'.replace( '%price%', buybackprice ) );
						$(buybackid).show();
					}
				},
		...
		if ( !bIsTrading )
		{
			var elSellButton = CreateMarketActionButton('green', 'javascript:SellCurrentSelection()', '出售' );
...
function SellCurrentSelection()
{
	...
	{
		MessageDialog.Show(
			'<a href="https://store.steampowered.com/steamaccount/addfunds" target="_top">向您的 Steam 钱包充值</a>或者在 Steam 商店中购买物品并提供账单地址之前，您不能在社区市场出售物品。',
			'无法出售物品'
		);
	}
	else
...
SellItemDialog = {
	...
	Show: function ( item ) {
		...
		$('market_sell_dialog_title').update( '物品上架出售' );
	...
	OnCanBuyBackSuccess: function( transport ) {
		...
		if ( transport.responseJSON && transport.responseJSON.canbuyback && transport.responseJSON.price )
		{
			...
			$('market_sell_buyback_details').update( '您可以将此卡牌售还给 Artifact，售价：' );
	...
	OnBuybackAccept: function( event ) {
		...
		{
			this.DisplayError( '您必须同意《Steam 订户协议》中的条款才能出售此物品。' );
			return;
		}
	...
	OnBuyBackSuccess: function( transport ) {
		...
		{
			this.DisplayError( '您的物品在上架时出现问题。请刷新页面并重试。' );
		}
	},
	...
	OnAccept: function( event ) {
		...
		if ( quantity < 1 || quantity > this.m_item.amount )
		{
			...
			this.DisplayError( '您必须输入一个有效的数量。' );
			return;
		}
		...
		$('market_sell_dialog_title').update( '确认售价' );
	...
	OnSuccess: function( transport ) {
		...
		if ( transport.responseJSON )
		{
			...
			if ( transport.responseJSON.requires_confirmation )
			{
				...
				{
					strText = '若要在社区市场内上架此物品，您必须在 Steam 手机应用中验证上架物品。您可以通过启动应用并导航至菜单的确认页面来验证。' +
						'<br><br>' +
						'如果您没在应用的主菜单上看到“确认”选项，那么请确保您已安装了最新版本的应用。';

				}
				else
				{
					strText = '想要在社区市场上售出该物品，您必须完成一个额外的验证步骤。对于该验证的说明已发送到您（结尾是“%s”）的电子邮件地址。'.replace( /%s/, transport.responseJSON.email_domain );
				}
...
{
	var $Dialog = $J('<div/>');
	$Dialog.append( $J('<div/>', {'class': 'trade_scam_report_label' }).html( '使用此功能来向 Steam 客服人员举报 %s 试图欺诈。<br><br>请提供简要说明事件经过：'.replace( /%s/, strPersonaName ) ) );
	var $TextArea = $J('<textarea/>', {'rows': 3, 'cols': 26 });
	$Dialog.append( $J('<div/>', {'class': 'gray_bevel fullwidth' }).append( $TextArea ) );
	var Modal = ShowConfirmDialog( '举报欺诈', $Dialog, '举报欺诈').done( function() {
		if ( !$TextArea.val() )
		{
			ShowAlertDialog( '举报欺诈', '需要提供描述。' );
			return;
		}
		...
		$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', rgParams).done( function() {
			ShowAlertDialog( '谢谢！', '您的举报已被发送并由 Steam 客服进行审核。' );
		}).fail( function() {
			ShowAlertDialog( '举报欺诈', '在保存您的举报时出现了一个问题。请稍后再试。' );
		});
	} );
...
function ShowEscrowExplanationDialog( bAddSteamGuardLink )
{
	...
	description.append( '<div class="escrow_explanation_q">交易或市场暂挂是什么？</div>' );
	description.append( '<div class="escrow_explanation_a">暂挂是指交易完成或市场物品上架前，物品由 Steam 暂挂的一段时间。</div>' );

	description.append( '<div class="escrow_explanation_q">物品暂挂帮助保护您的物品</div>' );
	description.append( '<div class="escrow_explanation_a">因为 Steam 帐户非常宝贵，尤其是在帐户中含有值得偷盗的物品的情况下。如果您没有通过物理设备（Steam 令牌手机验证器）保护您的帐户，交易暂挂将留给您发现帐户遭受入侵的时间，以免您的物品从帐户中消失。</div>' );

	description.append( '<div class="escrow_explanation_q">用以发现和阻止物品盗窃的缓冲时间</div>' );
	description.append( '<div class="escrow_explanation_a">在帐户上手机验证器启用未满 7 天的用户：<br><span style="color: #5aa9d6">交易：</span>物品在交易确认后将受 Steam 暂挂延迟送达，为时最高 15 天<br><span style="color: #5aa9d6">市场：</span>物品销售将受 Steam 暂挂延迟上架，为时最高 15 天<br><br>这将给予用户时间来取消任何未授权的待完成交易。<br><br>取消任何待确认或任何于托管期的交易，均将使帐户进入交易冷却期，保护帐户不受任何进一步未经授权的交易尝试。从市场上将物品下架则不受影响。</div>' );

	description.append( '<div class="escrow_explanation_q">移除物品暂挂的必要性</div>' );
	description.append( '<div class="escrow_explanation_a">使用手机验证器将确保您，且只有您，才能安全快捷地交易或出售自己的物品，所以暂挂也就相对变得不必要了。您可以通过下载 iOS 和 Android 设备的 <a href="https://store.steampowered.com/mobile/" target="_blank">Steam 令牌手机验证器</a>来提升您的帐户安全等级。</div>' );

	description.append( '<div class="escrow_explanation_a"><a href="http://store.steampowered.com/news/20631/" target="_blank">阅读关于帐户安全性的 Steam 博客</a>以了解我们为何实施暂挂政策的详细解释。</div>' );
